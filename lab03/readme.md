...
1. **Навигация по сайту**  
   В боковой панели темы реализована простая навигация, включающая ссылки на главную страницу и записи блога. Это позволяет пользователю быстро перемещаться между основными разделами сайта без необходимости возвращаться на главную страницу.

2. **Комментарии пользователей**  
   Для каждой записи и страницы подключён шаблон комментариев, который позволяет пользователям просматривать существующие комментарии и добавлять новые. Это обеспечивает интерактивность сайта и вовлечённость аудитории.

3. **Ограничение количества записей**  
   На главной странице выводятся только последние пять записей. Это упрощает восприятие информации пользователем и делает главную страницу более удобной и структурированной.

4. **Повторное использование шаблонов**  
   Использование общих частей шаблонов (header.php, footer.php, sidebar.php) позволяет быстро модифицировать элементы сайта в одном месте без необходимости правки всех страниц вручную. Это улучшает поддержку и масштабируемость темы.

5. **Стилизация и визуальная структура**  
   Применение сетки (grid) и карточек для записей, страниц и блоков навигации обеспечивает аккуратное и современное оформление. Это повышает удобство работы с контентом и визуальное восприятие сайта.

6. **Функциональность темы через functions.php**  
   Файл functions.php позволяет централизованно подключать стили, расширять возможности темы и добавлять поддержку новых функций без вмешательства в основной код шаблонов.

---

### Вывод

В ходе выполнения лабораторной работы была разработана полноценная пользовательская тема WordPress. Были созданы все обязательные файлы темы — style.css и index.php, а также дополнительные шаблоны: header.php, footer.php, sidebar.php, single.php, page.php, archive.php и comments.php. С помощью файла functions.php была подключена стилизация темы и обеспечена возможность расширения функциональности. На главной странице корректно выводятся последние записи блога с ограничением в пять записей, реализована навигация через боковую панель, подключён шаблон комментариев, что обеспечивает интерактивность сайта. В теме применены современные подходы к стилизации контента с использованием сетки и карточек, обеспечена единая визуальная структура сайта, удобная для пользователя. Созданный скриншот темы позволяет правильно отображать её в административной панели WordPress. В результате работы были изучены принципы работы WordPress Loop, подключение общих шаблонов, организация структуры темы и базовая стилизация элементов. Созданная тема является функциональной, аккуратной и готовой к дальнейшему развитию.
